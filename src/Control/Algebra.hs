...
instance (Algebra sig m, Handles (Either e) sig) => Algebra (Error e :+: sig) (Except.ExceptT e m) where
...
instance (Algebra sig m, Handles (RWSTF w s) sig, Monoid w) => Algebra (Reader r :+: Writer w :+: State s :+: sig) (RWS.Lazy.RWST r w s m) where
...
instance (Algebra sig m, Handles (RWSTF w s) sig, Monoid w) => Algebra (Reader r :+: Writer w :+: State s :+: sig) (RWS.Strict.RWST r w s m) where
...
instance (Algebra sig m, Handles ((,) s) sig) => Algebra (State s :+: sig) (State.Lazy.StateT s m) where
...
instance (Algebra sig m, Handles ((,) s) sig) => Algebra (State s :+: sig) (State.Strict.StateT s m) where
...
instance (Algebra sig m, Handles ((,) w) sig, Monoid w) => Algebra (Writer w :+: sig) (Writer.Lazy.WriterT w m) where
...
instance (Algebra sig m, Handles ((,) w) sig, Monoid w) => Algebra (Writer w :+: sig) (Writer.Strict.WriterT w m) where
