...
instance (Algebra sig m, Handles (Either String) sig) => Fail.MonadFail (FailC m) where
  fail = send . Fail
  {-# INLINE fail #-}

instance (Algebra sig m, Handles (Either String) sig) => Algebra (Fail :+: sig) (FailC m) where
