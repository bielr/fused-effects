...
instance (Carrier sig m, Handles (Either e) sig) => Carrier (Error e :+: sig) (Except.ExceptT e m) where
...
instance (Carrier sig m, Handles (RWSTF w s) sig, Monoid w) => Carrier (Reader r :+: Writer w :+: State s :+: sig) (RWS.Lazy.RWST r w s m) where
...
instance (Carrier sig m, Handles (RWSTF w s) sig, Monoid w) => Carrier (Reader r :+: Writer w :+: State s :+: sig) (RWS.Strict.RWST r w s m) where
...
instance (Carrier sig m, Handles ((,) s) sig) => Carrier (State s :+: sig) (State.Lazy.StateT s m) where
...
instance (Carrier sig m, Handles ((,) s) sig) => Carrier (State s :+: sig) (State.Strict.StateT s m) where
...
instance (Carrier sig m, Handles ((,) w) sig, Monoid w) => Carrier (Writer w :+: sig) (Writer.Lazy.WriterT w m) where
...
instance (Carrier sig m, Handles ((,) w) sig, Monoid w) => Carrier (Writer w :+: sig) (Writer.Strict.WriterT w m) where
